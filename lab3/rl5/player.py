...
def epsilon_greedy(Q,
    ...
    if eps_type == 'constant':
        ...
        random_number = np.random.uniform(0, 1)
        epsilon_t = linear_schedule.value(current_total_steps)
        if random_number < epsilon_t:
            action = np.random.choice(all_actions)
        else:
            action = np.nanargmax(Q[state, :])
        # ADD YOUR CODE SNIPPET BETWEENEX  4.2
...
class PlayerControllerRL(PlayerController, FishesModelling):
    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max:
            ...
            #print(R_values)
            # Check if the last 10 episodes have the same reward
            if len(R_values) == N_same and len(set(R_values)) == 1:
                break
            if episode % N_same == 0:
                R_values.clear()
            # ADD YOUR CODE SNIPPET BETWEEN EX. 2.3
